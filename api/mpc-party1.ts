...
import { party1, SecureValue } from './party1';
 ...
 */
function verifyTimestamp(timestamp: number, windowMs = 2 * 60 * 1000): boolean {
 ...
 */
function validateAndRecordNonce(nonce: string): boolean {
 ...
 */
async function fetchPartialKeyFromMainWebsite(keyId: string): Promise<{success: boolean; key?: string; reason?: string}> {
  try {
    ...
    const data = await response.json() as { partialKey?: string; error?: string };
...
export default async function handler(req: Request, res: Response): Promise<void> {
  ...
  if (req.method === 'GET') {
    const keyId = (req.query.keyId || req.query.key_id) as string;
    ...
    try {
      const partialKey = party1.getPartialKey(keyId);
      ...
      if (fetchResult.success && fetchResult.key) {
        ...
        party1.storePartialKey(keyId, fetchResult.key);
        res.status(200).json({
          success: true,
          operation: 'retrieve_partial_key',
          keyId,
  ...
  try {
    ...
    // Parse request body
    const {
      shareId,    // For storing pre-split share
    ...
    const signature = req.headers['x-mpc-signature'] as string;
    ...
    if (shareId && party1Share) {
      try {
        party1.storeShareHex(shareId, party1Share);
    ...
    if (keyId && partialKey) {
      try {
        party1.storePartialKey(keyId, partialKey);
